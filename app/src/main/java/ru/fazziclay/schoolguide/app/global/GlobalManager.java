...
public class GlobalManager {
    public static void getInExternalThread(SchoolGuideApp app, GlobalManagerInterface globalManagerInterface) {
        Thread thread = new Thread(() -> getInCurrentThread(app, true, globalManagerInterface));
        thread.setName("GlobalManager-get");
        thread.start();
    }

    public static void getInCurrentThread(SchoolGuideApp app, boolean onlineMode, GlobalManagerInterface globalManagerInterface) {
        try {
            ...
            if (onlineMode && globalKeys.versionManifest != versionManifest.getGlobalKey()) {
            ...
            if (onlineMode && globalKeys.builtinSchedule != builtinSchedule.getGlobalKey()) {
